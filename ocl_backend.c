...
/* sam_kernels.cl, turned into a string literal by the Makefile. */
static const char *sam_kernel_source =
#include "sam_kernels_cl.h"
    ;
...
static int try_device(struct ocl_backend *ocl, const struct candidate *c,
                      char **log)
{
    const char *src = sam_kernel_source;
    ...
    /* Developer aid: I_SAM_OPENCL_PROFILE=1 times every kernel. */
    ocl->profile = getenv("I_SAM_OPENCL_PROFILE") != NULL;
...
cl_mem ocl_alloc(struct ocl_backend *ocl, size_t n, const char *what)
{
    ...
    if (n > ocl->max_alloc)
        G_fatal_error(_("OpenCL: buffer <%s> needs %lu MiB, more than the "
                        "device maximum allocation of %lu MiB (reduce "
                        "batch= or tile_size=)"),
